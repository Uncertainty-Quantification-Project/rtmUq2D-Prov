...
int reading_file2(char *path, float *matrix, int dim1, int dim2, int identifier) {
	...
	fseek (file, sizeof(float)*identifier*(dim1*dim2), SEEK_SET);
	nR = fread (matrix, sizeof(float), dim1*dim2, file);
...
int createFileName(char *fileName, int size, int number) {
	...
	cx = snprintf(fileName, size, "seismogram%d.bin", number);
...
int recording_fileAppend(char *path, float *matrix, int dim1, int dim2) {
	...
	if (file == NULL) {
		printf("\nError oppening file (fopen in recording_fileAppend function)\n");
		exit(1);
	}
	...
	if (nW != dim1*dim2) {
		printf("\nError reading file (fread in recording_fileAppend function)\n");
		exit(1);
	}
