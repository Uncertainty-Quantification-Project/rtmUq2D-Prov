...
void adjointModeling(int simId, int *shot, float *velocity, float *wavelet, float *seismogram, float *imageCondition, float *selfImageCondition) {
    ...
    FILE *filePointerWavefield;
    FILE *filePointerWavefield2;
    
    char sourceWavefieldFile_02[1000];
...
//     filePointerWavefield  = fopen ("./TEMPORARY_FILES/receiverWavefield.bin","wb"); //Abrir arquivo para escrever em binario "wb"-> "write binary"
    ...
    controler = (numberOfTimeStep / 20) - 1;
...
// 	    cross_correlation(imageCondition, sourceWavefield, wavefield02);
...
//         if( (time + 1) % 20 == 0 ) {
//             for(i = thicknessLeft; i < dimX-thicknessRight; i++) {
//                 for(k = thicknessUp; k < dimZ-thicknessDown; k++) {
//                     
//                     fwrite ((wavefield02 + k + i*dimZ), sizeof(float), 1, filePointerWavefield);
//                 }
//             }
//         }
